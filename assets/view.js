  ...
  fetch("/api/v1/containers/info/" + challenge_id, {
    ...
    .then((data) => {
      ...
      if (data.status == "not_found") {
        alert.innerHTML =
          "No active instance. Click 'Fetch Instance' to start.";
        hideChallengeUpdate();
  ...
  function checkAndInject() {
    ...
    if (
      ...
      if (!challengeId && challengeWindow) {
        const titleElement = challengeWindow.querySelector(
          "[data-challenge-id]",
        );
        if (titleElement) {
          challengeId = parseInt(
            titleElement.getAttribute("data-challenge-id"),
          );
...
function renderConnectionInfo(connection, parent) {
  // Traefik compose mode: show all URLs from router rules
  if (connection.traefik_urls && connection.traefik_urls.length > 0) {
    connection.traefik_urls.forEach(function (item) {
      let link = document.createElement("a");
      link.href = item.url;
      ...
      if (item.name) {
        ...
        parent.append(link, label, document.createElement("br"));
      } else {
