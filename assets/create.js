CTFd.plugin.run((_CTFd) => {
  ...
  function parseFlagPattern() {
    ...
    if (!patternInput || !preview) {
      console.warn(
        "[Container Plugin] Missing elements - patternInput or preview",
      );
      return; // Guard clause
    }
  ...
  (function () {
    ...
    if (scoringTypeSelect) {
      ...
      try {
        ...
        console.error(
          "[Container Plugin] Error dispatching scoring type event:",
          e,
        );
      }
    } else {
  ...
  var containerImageDefault = document.getElementById(
